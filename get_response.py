...
class agent_response:
    ...
    async def _process_response(self, response: str, docs=None):
        ...
        human_match = re.search(human_pattern, response_wo_think)  # 搜索思考过程
        if human_match:
            human_content = human_match.group(1).strip()  # 提取思考内容
            response_wo_think = re.sub(human_pattern, '', response_wo_think).strip()  # 移除思考部分
        else:
            human_content = None
    ...
    # 入口处：运行应用
    # 在 app.py 中确保 run 方法返回响应
    async def run(self, prompt): 
        # print("history: ", self.get_history_summary())  
        if prompt:
            await self.process_user_input(prompt)  # 处理用户输入
            return self.get_last_response()  # 确保返回响应结果
        return "请输入有效的问题"
...
if __name__ == "__main__":
    ...
    response = asyncio.run(app.run('想转人工')) # 运行应用
    print(f"响应: {response}")